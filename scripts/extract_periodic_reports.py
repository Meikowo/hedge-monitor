...
def run_extraction_batch(
    ...
    for i, report in enumerate(reports, 1):
        ...
        if should_skip_reviewed(
            review_status_by_report.get(report_id),
            force_reviewed,
        ):
            log(
        ...
        try:
            ...
            record_report_failure(report, completed, exc, dry_run)
            failures.append({
                "report_id": report_id,
            ...
            if len(failures) >= failure_limit:
                ...
                warn(
                    f"本批累计 {len(failures)} 份失败，触发模型熔断；"
                    f"剩余 {remaining} 份不再调用 LLM"
                )
                break
            continue

        if not is_full_run:
            restore_report_state(report, dry_run)
        completed.append(report_run)
...
def finalize_extraction_batch(batch: dict) -> None:
    """先持久化完整诊断，再用非零退出明确标记熔断。"""
    snapshot_json("periodic_extract_run", batch)
    if batch["circuit_breaker"]:
        raise SystemExit(
            f"本批累计 {len(batch['failures'])} 份失败，已熔断后续 LLM 调用"
        )
...
def main() -> None:
    ...
    finalize_extraction_batch(batch)
