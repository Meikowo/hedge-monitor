(() => {
  ...
  const EVENT_FIELDS = [
    ...
    "approval_level", "first_ann_date", "latest_ann_date", "ann_count", "ann_roles",
    "instruments", "underlyings", "venue", "period_text", "is_revolving",
    "use_own_funds", "quota", "quota_source_ann_id", "built_at", "ind_l1", "ind_l2",
    "ind_l3", "ent_type", "province", "market", "scale"
  ].join(",");
  const ANNOUNCEMENT_FIELDS = [
    "ann_id", "event_key", "code", "name", "title", "ann_date", "pdf_url", "source",
    ...
    "ind_l2", "ind_l3", "ent_type", "province", "market", "scale"
  ].join(",");
  ...
  async function apiRows(path, params = {}) {
    ...
    const response = await fetch(url, { headers: { apikey: config.supabaseKey } });
    if (!response.ok) {
      const message = await response.text().catch(() => "");
      throw new Error(`数据接口 ${response.status}${message ? `：${message.slice(0, 80)}` : ""}`);
    }
  ...
  async function loadCoreData() {
    ...
    try {
      const [events, statuses] = await Promise.all([
        apiAll("v_events", { select: EVENT_FIELDS, order: "latest_ann_date.desc,event_key.asc" }),
        apiAll("announcements", { select: "status", order: "ann_id.asc" })
      ]);
      state.events = events;
  ...
  function renderMetrics() {
    ...
    const extractedCount = statusCount("extracted");
  ...
  function renderDashboard() {
    ...
    const quality = [
      { label: "额度已披露", count: state.events.filter((row) => asArray(row.quota).length > 0).length },
      { label: "工具字段", count: state.events.filter((row) => asArray(row.instruments).length > 0).length },
      { label: "品种字段", count: state.events.filter((row) => asArray(row.underlyings).length > 0).length },
      { label: "期限字段", count: state.events.filter((row) => Boolean(row.period_text)).length }
    ];
    $("#quality-chart").innerHTML = quality.map((item) => `<div class="quality-item"><span>${escapeHtml(item.label)}</span><strong>${(item.count / total * 100).toFixed(1)}%</strong><small>${item.count.toLocaleString("zh-CN")} / ${state.events.length.toLocaleString("zh-CN")} 个事件</small></div>`).join("");
  }
  ...
  function renderEvents() {
    ...
    }).join("") : '<tr class="empty-row"><td colspan="8">没有匹配的事件，请调整搜索或筛选条件。</td></tr>';
  ...
  function renderAnnouncements() {
    ...
    }).join("") : '<tr class="empty-row"><td colspan="7">没有匹配的公告，请调整搜索或筛选条件。</td></tr>';
    renderResultMeta(rows.length);
    renderPagination(rows.length);
  }
  ...
  function exportCurrentResults() {
    ...
    const headers = isEvents
      ? ["最新披露", "年度", "股票代码", "公司", "一级行业", "二级行业", "企业性质", "类别", "事件阶段", "审批层级", "工具", "品种", "交易场所", "期限", "额度循环", "自有资金", "额度明细", "关联公告数", "事件键"]
      : ["披露日期", "股票代码", "公司", "公告标题", "公告角色", "审批层级", "类别", "工具", "品种", "期限", "行业", "企业性质", "置信度", "证据数", "摘要", "原文链接", "公告ID"];
