# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.7
...
> 最后更新：2026-08-01（全项目进度复核 + 风险案例监控前端设计）
...
| M6a | 自动化衍生品风险案例 POC（PRD 5.8） | 🔄 官方四表与 SSE 适配器已上线；私有 Tavily 新闻线索每日一次增量监测，并新增 2000—2025 年可续跑历史回填；截至本次复核正式案例与媒体线索均为 0，待首轮回填及 SZSE/CSRC、LLM 官方证据抽取 |
| M6b | 风险案例历史扩展、增量监测与前端（PRD 7.8） | 🔄 风险案例监控工作区设计已确认；先制作本地 Demo，达到 M6a 正式案例与证据质量闸门后再开放线上入口 |
...
- 数据来源：交易所问询/监管/纪律处分、证监会及派出机构处罚/监管…657 tokens truncated…本次发布，
