# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.9
...
| M6a | 自动化衍生品风险案例 POC（PRD 5.8） | 🔄 官方四表与 SSE 适配器已上线；正式官方案例仍为 0；媒体公开层经严格闸门保留 2 条并撤回 2 条误报，下一步完成 1—3 个官方案例纵向切片 |
| M6b | 风险案例历史扩展、增量监测与前端（PRD 7.8） | ✅ 正式前端 v1 已实现：官方/媒体双证据层、筛选、高密度列表、详情和 CSV；50 份候选/10 案例/90% 精确率保留为官方管线扩量闸门，不再阻塞真实媒体层上线 |
