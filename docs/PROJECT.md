# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.1

> 用途：每次与 Claude 开新会话时上传本文件（或放入 Claude Project 知识库）。
> 由你维护；每次会话结束让 Claude 输出更新段落，你替换后 commit。
> 需求的唯一基准是 docs/PRD.md（v1.3），本文件记录"现状与决策"，不复述需求。
> 最后更新：2026-07-25（衍生品风险案例库立项 + M4/M6 并行边界）
...
- 当前状态：需求与架构已写入 PRD v1.3；尚未创建风险案例数据库迁移、采集脚本、
  Actions 工作流或前端入口。
