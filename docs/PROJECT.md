# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.15

> 用途：每次与 Claude 开新会话时上传本文件（或放入 Claude Project 知识库）。
> 由你维护；每次会话结束让 Claude 输出更新段落，你替换后 commit。
> 需求的唯一基准是 docs/PRD.md（v1.8），本文件记录"现状与决策"，不复述需求。
> 最后更新：2026-09-23（年报新增完成10家、补回并定位37份、修复标题/代码漏检、M5来源闸门）
> 当前进度以第 6 节和最新检查点为准；后文旧日期检查点保留为历史记录。
...
- **前端**：GitHub Pages 静态站（M3 重做），数据通路定为 **anon key 直连
  ...
  v_ann_flow / v_events。设计为浅白底、细边界、高密度研究表格与右侧证据抽屉；
  主网址为 https://www.hedgemonitor.site/，已绑定 GitHub Pages 并启用 HTTPS。
- **调度**：daily（北京03:00）负责公告采集；extract 每6小时自动抽取最多600条
  pending；audit（每月1日）自动补漏；backfill / import-companies / probe 手动触发。
- **年报调度**：2025FY 固定正式池 1,812 家；每天一次元数据发现、每 6 小时定位最多
  48 家并抽取最多 36 家。2026-09-08 普通队列为空，失败/OCR/未发现项仍需专项收尾。
...
## 6. 进度清单（基线 2026-09-08；运行与 M4b/M5 更新于 2026-09-23）
...
| R1 | 2026 公告日更与事件重建 | ✅ 初次验收已完成；当前 pending=0，另有 4 条新增 failed 待处理 |
| R2 | 2025→2021 公告历史回填与抽取 | 🔄 五年均已回填且 pending=0；2022—2023 合计 7 条 failed 待收尾 |
| R3 | 50 份金标准、字段级准确率 | ⏸ 尚未形成完整评测验收，不能以程序回验率替代人工准确率 |
| M3 | 高密度研究前端 | ✅ 主体已部署：事件/公告、证据、看板、筛选及 CSV；独立公司页与关注列表仍待实现 |
| M4a | 年报采集与解析 | 🔄 2025FY 固定证券代码池已抽取 1,765/1,812，37份已定位待抽取；发现4组新旧代码重复，不能再将代码分母等同独立公司数；2021—2024 年报未启动 |
| M4b | 半年报采集与解析 | 🔄 2026H1 六家公司已发现/定位，完成两轮 dry-run；修复跨页误接表、授权额误当实际峰值、折算差额误当套期 OCI。事实未正式入库，仍待漏提与语义核验，不启动全市场中报定时批量 |
| M5 | 计划 vs 实际三维核对 | 🔄 已交付确定性预核对规则与十二家公司、十九业务行的本地复核材料；未上线自动结论。数值候选存在期间、业务归属、单位及实际/授权混淆，先补证据后落独立派生表 |
| M6a | 自动化衍生品风险案例 | 🔄 7 个官方案例；93 份来源文档中有 2 份候选；官方来源覆盖及 50 候选/10 案例/90% 精确率闸门尚未完成 |
| M6b | 风险历史、增量与前端 | 🔄 正式前端、定时采集与 CSV 已上线；公开媒体 3 条、已获官方佐证 1 条，历史完整覆盖仍待扩展 |

当前数据库：公司 5,568 家、公告 25,062 条、相关结构化公告 21,800 条、事件 9,408 个。
2025FY 有 1,769 条报告摘要、对应 1,766 家公司（含早期池外样本及同公司不同版本），
不能将报告行数当作正式池完成公司数。数值事实 7,107 条，全部通过数字和引文回验；
正式池明确记录人工接受的公司仅 6 家，仍需扩量后的分层抽检。

2026-09-23本轮清理后，固定代码池待处理47项 = 已定位37 + 抽取失败1 + 需要OCR1 +
旧代码下未发现8。后8项中4项属于同公司新旧代码同时入池、2项为A/B股共用报告关联，
其余2项需结合披露状态/吸收合并证据核查。固定分母暂不改变；不伪造8份报告补足数量。
历史“57家公司异常”的称呼修正为57个代码层待处理项，详见最新检查点。

下一步：M5 先补授权生效日期与年内调额关系，明确报告级保证金的业务归属，
修复已识别的实际/授权混淆后再生成可公开的独立派生结果、接入页面及导出。
2026H1 六份试跑继续核对漏提、会计政策与实际应用的区别，不阻塞 2025FY 的 M5。
本轮已修复晶科明细表被风险说明页挤出的问题：保持15页预算，真实 PDF 重放恢复
商品/外汇各5条表格事实；尚不是完整试跑验收。金额上限过滤改为金额所在分句判断，
避免误删带“未超过授权额度”说明的实际峰值；先恢复原文单位再判断，且续表
识别不将“货币资金”当作“货币合约”。全量238项测试通过。

### 2026-09-23 第二轮实施检查点（优先于下方历史数字）

- 原8份JSON失败做一次有上限恢复，7份成功、英力特仍失败；未无限重跑。
  东方精工、公牛、阳光电源原先误选英文版/英文简版，找回中文全文后均完成抽取流程。
  本轮正式池从1755增至1765；这不是新增10份人工验收，阳光电源数值漏提仍需抽检。
- 发现器修复英文标题变体和下划线/空格分隔的报告标题；按巨潮相同orgId建立唯一的
  北交所920新代码至固定池旧代码映射，保存reported_code来源，不按名称猜测或合并A/B股。
  定向45个原未发现代码找回41份：37份新元数据已入库并完成定位，4份实际已在新代码下抽取。
- 原1812代码池同时包含力王股份、晨光电缆、贝特瑞、同享科技的新旧代码，至少有4组重复。
  未覆盖已有报告code或事实、未改变验收分母，后续进度统计应另列发行人去重与证券代码覆盖。
- 定位器显式传入多个report_id时不再被默认limit=2截断。37份located由原每六小时、
  每批36份的工作流继续处理；本轮不改调度、不另建无限重试。
- M5加入来源标题否决：第三方核查意见、法律意见、可行性分析等，即便旧抽取误标为计划
  角色，也不贡献额度或品种授权；被排除来源保留在只读审计明细，不改历史原始事实。
  12家19业务行已重算；还没有发布确定数值结论，期间和覆盖口径继续补证。
- 本轮不修改前端、schema/RLS或2026H1正式数据。执行细节见 worklog_2026-09-23-2.md。

### 2026-09-23 第一轮实施检查点

- 2025FY正式池仍为1755/1812家，待收尾57家；全库1769份已抽取报告，不能
  把定时任务success理解为每轮新增。普通队列为空，异常项不自动反复耗费模型额度。
- 最近100次Actions中15次失败，最新20次均成功。已查代表故障为巨潮HTTP403、
  上交所网络不可达；恢复后3天回看窗不能保证补齐连续失败期。
- 9月15—23日只读重新检索14条公告，确认6条缺失，已通过既有幂等写入函数补回
  pending（仅新公告元数据，不覆盖既有抽取状态）。总公告25085、事件仍9413；
  此时pending=6、failed=11；后续由原定时抽取处理，事件数需待抽取聚合后变化。
- 2026H1第三轮6份dry-run耗时约118秒，57条候选；晶科表格漏页修复生效。
  仍发现两条明确综合损益漏提，已补严格原文句式兜底，真实PDF重放验证：
  晶科-40313.15万元为报告级混合业务合计；株冶-26058614.68元为商品业务。
  修正中集摘要声称采用套期会计、但结构化结论未明确披露的矛盾。
- M5仍为12家19业务行预核对，未上线正式匹配；新增发现第三方核查意见角色误标，
  授权日期需关联股东大会决议而非公告发布时间。优先补证据，不放松判定标准。
- 本轮不改前端/定时配置，不写中报正式事实；细节见 worklog_2026-09-23.md。

### 2026-09-11 实施检查点

- `scripts/plan_actual_rules.py`：无网络、无写库的规则核心。按单位倍率、币种、
  业务类别、引文回验及绝对授权区间检查；B 仅报告日快照，A 仅完整覆盖期间的峰值。
  多额冲突、相对授权期和源事实疑似授权上限都拒绝自动比较。
- `scripts/audit_plan_actual.py`：只读抓取十二家公司 2025FY 及 2024—2025 计划类
  公告，保存完整来源快照与可重算 JSON/HTML。所有结果为预核对，不写数据库。
- 本轮 68 条候选额度中 8 条的绝对授权期通过 PDF 原页全文回验；19 个业务行中的
  10 个不同数值候选分别因业务归属不明 4、期间缺证 3、单位未识别 1、误提授权额 2
  被拦截。重复出现在多个业务行的报告合计不重复计数；未形成确定的数值匹配结论。
- 2026H1 复用晶科、中集、株冶、格力、大北农、博彦六家。初轮 54 条候选事实，
  修复后第二轮 44 条；两轮都是 dry-run，条数变化包含修复和模型漏提波动，
  不代表 44 条全部语义验收通过。库中仅六份 located 元数据，profiles=0。
- 线上 2025 报告与公告事实不回写。历史误提候选由 M5 规则先阻断；存量清理需
  单独列出 ID 和来源核验后处理。新提示词 `periodic-v2.5-actual-vs-authorization`，
  PDF 定位/表处理版本 `v2.4`。原 2025FY 每日发现、每六小时队列调度保持不变。
- 本次发现远端 `tests/test_periodic.py` 已截断损坏（30,012 字节），本地完整版本
  可运行；同步恢复完整测试文件并验证 Git blob 哈希，避免延续损坏文件。
- 详细证据、运行命令、剩余核验项与回滚见 `docs/worklogs/worklog_2026-09-11.md`。
...
## 46. M6a 交易所官方来源双轨扩展（2026-08-11）

- Track A 已在本地完成正式案例发布器的增量批次修复：先排除已经存在的 `cninfo:<ann_id>`，再应用批次上限；全部候选均已发布时作为正常空批次结束，不再让每日任务失败。生产发布后预计新增台华新材和焦作万方 2 个已通过 PDF 原文核验的案例，正式官方案例由 3 个增至 5 个；本检查点不提前宣称已写库。
- Track B 新增深交所官方来源适配器，并把采集器扩展为 `sse`、`szse`、`all` 三种模式。上交所继续使用官方查询接口；深交所经真实页面排查确认由浏览器调用 `/api/report/ShowReport/data`，因此适配器读取官网 JSON 报表接口及官方 PDF 主机，而不是误解析没有数据行的静态 HTML 外壳。
- 交易所采集只写 `risk_source_documents` 候选层，不会自动创建 `derivative_risk_cases`。单一来源失败时，另一来源的快照和成功写入仍被保留，失败正文标记为 `failed`。每天北京时间 10:10 自动运行最近 30 个自然日的重叠窗口；手工写入仍需 `write=true` 与 `I_UNDERSTAND` 双确认。
- 本地真实 dry-run：上交所 33 份文档，0 候选、33 无关、0 失败；深交所 35 份文档，1 候选（多氟多监管函）、34 无关、0 失败。正式风险页面的官方文档请求增加 `status=extracted`，候选池增长不会被浏览器整池读取。
- 发布候选验证：211 项 Python 测试、7 组离线 Node 测试全部通过；真实风险页启动为 3 个官方案例、2 条媒体记录、2 个媒体来源，约 2.8 秒；主站启动为 9,221 个事件和 20,546 条结构化公告。GitHub 发布、Actions 写入、5 案例生产核验、幂等复跑及 Security Advisor 结果记录在 `docs/worklogs/worklog_2026-08-11-3.md`，未完成项不得提前写成成功。

## 47. M4 年报自动任务吞吐优化（2026-08-13）

- 2025FY 正式池当前公司级进度为：目标 1,812 家，已发现 1,767 家，已抽取 378 家，已定位待抽取 448 家，待定位 938 家，待发现 45 家；另有 1 家抽取失败和 1 家需要 OCR。生产前端包含早期池外样本，因而当前展示公司总数为 389 家。
- 运行记录确认旧任务每天四轮均稳定抽取 18 家，8 月 10—12 日每天各新增 72 家；但每轮约 1 小时 50 分钟主要耗在重复全市场元数据扫描，真正的 18 家 LLM 抽取通常只需约 8 分钟。
- 自动调度拆分为两种模式：每天北京时间 05:20 仅执行一次全市场 2025 年报元数据扫描；北京时间 00:45、06:45、12:45、18:45 的四轮队列仅执行进度快照、定位最多 48 家和串行抽取最多 36 家。
- 手工 metadata/locate/extract 入口、cninfo 并发锁、三次失败熔断、单报告失败隔离、显式失败重试、LLM_THINKING=off、正式池范围和数据库结构均保持不变。理论抽取上限由 72 家/天提高到 144 家/天，实际吞吐需由发布后的 Actions 记录确认。
