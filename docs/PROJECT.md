# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.12
...
> 最后更新：2026-09-08（全项目进度同步；M5 首期限定 2025FY）
...
| M4b | 半年报采集与解析 | ⏸ 尚未启动正式数据批次 |
| M5 | 计划 vs 实际三维核对 | 🔄 用户确认首期使用 2025 年报；已核对数据条件并落实首期范围，自动核对代码/派生结果/前端尚未交付；现站仍展示候选关联 |
...
下一步：在已完成的 2025FY 报告上进入 M5，先验证品种/模式匹配和少量同口径数值案例，
生成有理由和证据的独立派生结果，再接入现有页面与导出。年报异常收尾和人工抽检并行；
跨年报与半年报扩量另行推进，不再作为 M5 首期的前置条件。
