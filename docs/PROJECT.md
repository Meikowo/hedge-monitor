# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.3
...
> 最后更新：2026-07-26（M4 v1.5 数据契约与六份年报验证批次）
...
| M4a/b | 定期报告（年报+半年报）采集与解析 | 🔄 M4a：30份2025FY元数据；6份验证批次已抽取，流程通过、质量闸门未通过 |
...
- 会计政策和业务表不再作为实际应用套期会计的充分证据；空白“现金流量套期储备”模板
  不得误判为当期应用。
- 六份样本共 72 条数值事实，72 条全部双回验：中集 18、格力 14、大北农 10、
  博彦 0、株冶 13、晶科 17。
