# PROJECT.md —— 套保监控（hedge-monitor）项目上下文主文件 v2.14
...
> 最后更新：2026-09-23（运行复核、补回6条漏抓公告、中报综合损益与摘要一致性修复）
...
| M4a | 年报采集与解析 | 🔄 2025FY 正式池已抽取 1,755/1,812 家（96.9%）；普通队列为空，待收尾 57 家；2021—2024 年报未启动 |
...
正式池待收尾 57 家 = 未发现 45 + 抽取失败 8 + 需要 OCR 2 + 当前规范状态为 skipped 2。
后两家需要核对跳过原因及有效版本，不直接计作已完成，也不能只因 skipped 就认定重复。
